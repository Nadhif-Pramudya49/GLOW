  ...
  wrap.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.5rem">
      ...
      <div class="form-group">
        ...
        <input type="date" class="form-input" value="${bookingState.checkOut}" readonly style="background:var(--gray-50)" />
      </div>
    </div>
    ...
    <div style="background:var(--gray-50);border-radius:12px;padding:1.5rem;margin-bottom:1.5rem">
      ...
      ${pkg.penginapan ? `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:0.75rem;background:#fff;border-radius:8px;margin-bottom:0.5rem">
          <div><span style="font-weight:600">🏨 ${pkg.penginapan.name}</span><br/><span style="font-size:0.75rem;color:var(--gray-400)">${pkg.nights} malam</span></div>
          <div style="display:flex;align-items:center;gap:0.75rem">
            <span style="color:#16a34a;font-weight:600">✓ Tersedia</span>
            <span style="font-weight:700">${formatRupiah(pkg.penginapan.price*pkg.nights)}</span>
          </div>
        </div>
      ...
      ${pkg.transport ? `
        <div style="display:flex;justify-content:space-between;padding:0.75rem;background:#fff;border-radius:8px;margin-bottom:0.5rem">
          ...
          <span style="font-weight:700">${formatRupiah((pkg.transport.price||0)*pkg.nights)}</span>
        </div>
      ` : ''}
...
function setCheckIn(val) {
  ...
  d.setDate(d.getDate() + (State.package.nights || 3));
