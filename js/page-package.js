...
function renderPkgPenginapan() {
  ...
  const content = `
    <div style="margin-bottom:1rem;display:flex;align-items:center;gap:1rem">
      <label style="font-size:0.875rem;font-weight:600;color:var(--gray-600)">Durasi Menginap:</label>
      <input type="number" class="form-input" style="width:80px" min="1" max="30" value="${pkg.nights}"
        onchange="State.package.nights=+this.value;renderApp()" />
      <span style="font-size:0.875rem;color:var(--gray-400)">malam</span>
    </div>
...
function renderPkgTransport() {
  const pkg = State.package;
  const content = `
    <p style="font-size:0.8rem;color:var(--gray-400);margin-bottom:1rem">Durasi otomatis mengikuti lama menginap (${pkg.nights} hari).</p>
      ...
      ${TRANSPORT_OPTIONS.map(t => {
        ...
        return `
          <div style="padding:1rem;border-radius:12px;border:2px solid ${sel?'var(--green)':'var(--gray-200)'};background:${sel?'rgba(26,74,58,0.04)':'#fff'};cursor:pointer;text-align:center;transition:all 0.2s" onclick="selectTransport('${t.id}')">
            ...
            ${t.price>0?`<div style="font-size:0.75rem;color:var(--gray-400)">Total: ${formatRupiah(t.price*pkg.nights)}</div>`:''}
          </div>
        `;
      }).join('')}
    </div>
...
function renderPackageSummary() {
  ...
  summaryEl.innerHTML = `
    ...
    <div class="package-breakdown">
      ...
      ${pkg.penginapan ? `<div class="breakdown-row"><span>🏨 ${pkg.penginapan.name}<br/><span style="font-size:0.75rem;color:var(--gray-400)">${pkg.nights} malam × ${formatRupiah(pkg.penginapan.price)}</span></span><span style="font-weight:700">${formatRupiah(pkg.penginapan.price*pkg.nights)}</span></div>` : `<div class="breakdown-row" style="color:var(--gray-400)"><span>🏨 Belum pilih penginapan</span><span>-</span></div>`}
      ...
      ${pkg.transport ? `<div class="breakdown-row"><span>${pkg.transport.icon} ${pkg.transport.name}<br/><span style="font-size:0.75rem;color:var(--gray-400)">${pkg.nights} hari × ${formatRupiah(pkg.transport.price||0)}</span></span><span style="font-weight:700">${formatRupiah((pkg.transport.price||0)*pkg.nights)}</span></div>` : `<div class="breakdown-row" style="color:var(--gray-400)"><span>🚗 Belum pilih transport</span><span>-</span></div>`}
      ...
      <div class="breakdown-row breakdown-total"><span>TOTAL</span><span style="color:var(--green)">${formatRupiah(total)}</span></div>
    </div>
