...
const el = (tag, cls) => { const e = {}; e.className = cls; e.appendChild = () => {}; return e; };
const State = { user: { fullName: 'Budi Santoso', role: 'USER' } };
const sessionStorage = { getItem: () => null };
const encodeURIComponent = encodeURI;
      ...
      listEl.innerHTML = bookings.map(b => {
        ...
        return `
          <div class="dashboard-card" style="margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center;position:relative;z-index:2;">
            <div>
              <div style="font-size:0.75rem;color:var(--gray-500);margin-bottom:4px">ORDER #${b.id} • ${b.status}</div>
              ...
              <div style="font-size:0.875rem;color:var(--gray-600)">${inDate} — ${outDate} (${b.guestCount} Tamu)</div>
            </div>
...
renderDashboardUser().then(console.log).catch(console.error);
